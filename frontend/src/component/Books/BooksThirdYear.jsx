...
const BooksThirdYear = () => {
  const firstSemesterCourses = [
    ...
    { title: "Data Structure and Algorithms", icon: "📊" },
    { title: "Computer Organization and Architecture", icon: "🖥️" },
    { title: "Internet Programming 1", icon: "🌐" },
    { title: "System Analysis and Modelling", icon: "📈" },
  ];

  const secondSemesterCourses = [
    { title: "Internet programming 2", icon: "🌐" },
    { title: "OperatingSystem", icon: "🐧" },
    { title: "Software requirements engineering", icon: "📝" },
    { title: "Advanced programming", icon: "🚀" },
    { title: "Flat", icon: "🤖" },
  ];
...
const CourseCard = ({ course }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const requestData = { course_name: course.title.trim() };
